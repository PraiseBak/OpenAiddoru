...
public class TagService {
    ...
    public List<Idol> recommendIdolByTag(String userId, Set<ZSetOperations.TypedTuple<Object>> tagSet) {
        ...
        for (ZSetOperations.TypedTuple<Object> tuple : tagSet) {
            ...
            boolean recommended = tagExtinst(idolList, redisUserId, resultArrayList);
            if(!recommended){
                idolList = getIdolsByTag(tagName,null);
            }
            //한개씩만 추천하도록 수정
    ...
    private boolean tagExtinst(List<Idol> idolList, String redisUserId, List<Idol> resultArrayList) {
    ...
    private List<Idol> getIdolsByTag(String idolTag, String country) {
        if(country == null){
            return tagRepository.findTop100ByIdolIsNotNullAndTagOrderByWeightDesc(idolTag).stream()
                    .map(Tag::getIdol)
                    .collect(Collectors.toList());
        }

        //idol tag가 일치하면서 idolId가 null이 아닌애들 가져오기
        return tagRepository.findTop100ByIdolIsNotNullAndTagAndIdolCountryOrderByWeightDesc(idolTag,country).stream()
                .map(Tag::getIdol)
                .collect(Collectors.toList());
    }
    ...
    public void setInitTagList(String userId) {
        String redisUserId = userId + RedisHelper.tagWeightSetUserIdPostfix;
        List<Tag> initTagList = tagRepository.findTop100ByOrderByWeightDesc();
